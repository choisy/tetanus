...
walk2(xs, ys, lines2)
abline(v = log(.1 / c(1, 50, 100, 200)), lwd = lwd_val, col = color_data)
```
...
#  axis(2, seq(mean(1:nb_dltns), nb_dltns * nb_smpls, nb_dltns), nb_smpls:1)
#  box(bty = "o")
}
...
axis(1)
axis(2, col = 2, col.axis = 2)
mtext("optical density", 2, line = 1.5, col = 2)
walk2(xs, ys, lines2)

par(new = TRUE)
with(anti_toxins_luminex,
     plot2(log(concentration), od, ylab = "MFI", xlim = xlim, lwd = lwd_val,
           axes = FALSE, ann = FALSE))
anti_toxins_luminex |> 
  group_by(concentration) |> 
  summarize(od = mean(od)) |> 
  with(lines(log(concentration), od, lwd = lwd_val, col = color_data))
axis(2, line = -3, col = 4, col.axis = 4)
mtext("median fluorescence intensity", 2, line = -1.5, col = 4)
```
